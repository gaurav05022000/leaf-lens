...
object ScanHistoryManager {
        ...
        auth?.currentUser?.let { user ->
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    db.collection("users").document(user.uid).collection("scan_history").document(item.id).set(map).await()
                    ...
                    if (!item.disease.isNullOrBlank() && item.disease != "None") {
                        val diseaseLog = map + mapOf("logType" to "Disease Identificaton", "userId" to user.uid)
                        db.collection("users").document(user.uid).collection("disease_logs").document(item.id).set(diseaseLog).await()
    ...
    fun syncWithFirestore() {
        auth?.currentUser?.let { user ->
            ...
            snapshotListener?.remove()
            snapshotListener = db.collection("users").document(user.uid).collection("scan_history")
                .addSnapshotListener { snapshot, error ->
                    ...
                    missingInCloud.forEach { item ->
                        ...
                        db.collection("users").document(user.uid).collection("scan_history").document(item.id).set(map)
                        db.collection("scan_history").document(item.id).set(map)
                        
                        if (!item.disease.isNullOrBlank() && item.disease != "None") {
                            val diseaseLog = map + mapOf("logType" to "Disease Identificaton", "userId" to user.uid)
                            db.collection("users").document(user.uid).collection("disease_logs").document(item.id).set(diseaseLog)
